...
let
  ...
  inherit (import ../../modules/home/lib.nix { inherit pkgs; }) waitForInternet;
in
{
  ...
  mkProjectService =
    {
      ...
      restartTriggers ? [ ], # restart on home-manager switch when these change
      oneshot ? false,
      timeoutStart ? null,
      # long-running services are node servers killed by SIGTERM
      timeoutStop ? 10,
    ...
    in
    {
      ...
      // lib.optionalAttrs (restartTriggers != [ ]) { X-Restart-Triggers = restartTriggers; };
