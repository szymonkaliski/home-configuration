...
in
{
  ...
  systemd.user.services.neolink = {
    Unit = {
      ...
      # restart on switch when the rendered config changes; hashes the template
      # text, which holds sops placeholders, not secrets
      X-Restart-Triggers = [
        (builtins.hashString "sha256" config.sops.templates."neolink-config".content)
      ];
    };
  ...
  systemd.user.services.neolink-dashboard = mkProjectService {
    ...
    restartTriggers = [
      (builtins.hashString "sha256" config.xdg.configFile."neolink-dashboard/config.json".text)
    ];
  };
}
