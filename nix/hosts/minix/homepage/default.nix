...
let
  ...
  indexHtml = ''
    ...
    async function refresh(){
      try{
        const r=await fetch(`http://''${location.hostname}:${toString glancesPort}/api/4/quicklook`,{cache:'no-store'});
        if(!r.ok)return;
        const d=await r.json();
        const set=(id,v)=>{const e=document.getElementById(id);e.textContent=v;e.classList.remove('dim');};
        set('cpu',Math.round(d.cpu)+'%');
        set('mem',Math.round(d.mem)+'%');
        set('swap',Math.round(d.swap)+'%');
      }catch(e){}
    }
    refresh();
