...
# adopt serves matching our shape (https=PORT proxying to 127.0.0.1:PORT)
# instead of `serve reset`, which would also wipe funnels created by agents
{ tailscale serve status --json 2>/dev/null || true; } \
  | jq -r '(.Web // {}) | to_entries[]
      | (.key | split(":")[1]) as $port
      | select(.value.Handlers["/"].Proxy == ("http://127.0.0.1:" + $port))
      | $port' \
  | while IFS= read -r port; do touch "$STATE_DIR/$port"; done
