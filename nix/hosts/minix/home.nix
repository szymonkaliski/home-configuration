...
in
{
  ...
  sops.templates."vitalist-config" = {
    path = "${config.home.homeDirectory}/.config/vitalist/config.json";
    content = builtins.toJSON {
      logsDir = "${config.home.homeDirectory}/.dropbox-hm/Dropbox/Health/Logs";
      whoop = {
        clientId = config.sops.placeholder.whoop_client_id;
        clientSecret = config.sops.placeholder.whoop_client_secret;
      };
      withings = {
        clientId = config.sops.placeholder.withings_client_id;
        clientSecret = config.sops.placeholder.withings_client_secret;
      };
    };
  };
  ...
  systemd.user.services.vitalist = mkProjectService {
    dir = "vitalist";
    description = "Vitalist sync";
    extraUnits = [ "sops-nix.service" ];
    needsInternet = true;
    command = "node src/index.ts sync";
    oneshot = true;
    timeoutStart = "10min";
  };

  systemd.user.timers.vitalist = mkTimer {
    description = "Vitalist sync timer";
    onCalendar = "*-*-* 12:00:00";
    randomizedDelay = "5m";
  };
